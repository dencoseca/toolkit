...
// RandomString returns a string with random characters of length n, using
	...
	for _, fHeaders := range r.MultipartForm.File {
		for _, hdr := range fHeaders {
			uploadedFiles, err = func(uploadedFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				uploadedFile.OriginalFileNme = hdr.Filename
...
// It also allows specification of a display name
...
// optional, if none is specified we use the standard htp.Client
func (t *Tools) PushJSONToRemote(uri string, data any, client ...*http.Client) (*http.Response, int, error) {
	...
	// check for a custom HTTP client
