	...
	for _, tt := range uploadTests {
		...
		if !tt.errorExpected {
			...
			if _, err := os.Stat(fileName); os.IsNotExist(err) {
				t.Errorf("%s: expected file to exist: %s", tt.name, err)
			}
		...
		if err == nil && tt.errorExpected {
			t.Errorf(fmt.Sprintf("%s: error expected but none recieved", tt.name))
		}
...
func TestTools_Slugify(t *testing.T) {
	...
	for _, tt := range slugTests {
		...
		if err != nil && !tt.errorExpected {
			t.Errorf("%s: error received when none expected: %v", tt.name, err)
		}
...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, tt := range jsonTests {
		...
		req, err := http.NewRequest(http.MethodPost, "/", bytes.NewReader([]byte(tt.json)))
		if err != nil {
			t.Error(err)
		}
		...
		if tt.errorExpected && err == nil {
			t.Errorf("%s: error expected but none recived", tt.name)
		}

		if !tt.errorExpected && err != nil {
			t.Errorf("%s: error not expected but one recieved: %s", tt.name, err.Error())
		}
...
func TestTools_ErrorJSON(t *testing.T) {
	...
	if rr.Code != http.StatusServiceUnavailable {
		t.Errorf("wrong status code: returned: expecteed %d, but got %d", http.StatusServiceUnavailable, rr.Code)
	}
}
